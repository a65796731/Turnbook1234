...
public class HapticGrabber : MonoBehaviour
{
    ...
    public List<Transform> GrabberItemList = new List<Transform>();
    ...
    public void CheckDrag(float angle)
    {
        IsDrag = angle >= 0.9f;
    }
    ...
    public GameObject GetNearObject()
    {
        ...
        foreach (Transform grabberItem  in GrabberItemList)
        {
            var tempdistance = Vector3.Distance(grabberItem.transform.position,transform.position);

            if (tempdistance > 0.2f)
                continue;
            foreach (Transform collider in grabberItem)
            {
                var tempdistanceCollider = Vector3.Distance(collider.transform.position, transform.position);
                if (tempdistanceCollider > 0.2f|| tempdistanceCollider<0.1f)
                    continue;
                result = grabberItem.gameObject;
            }
         }
        return result;
    }
    ...
    void FixedUpdate()
    {
        bool newButtonStatus = IsDrag;
        //if (!IsDrag&&touching!=null)
        //{
        //    touching = GetNearObject();
        //    return;
        //}
        //touching = GetNearObject();
        bool oldButtonStatus = buttonStatus;
        buttonStatus = newButtonStatus;
        if(IsDrag&&!IsArea&& touching)
        {
            IsArea = IsCollision(transform,touching.transform);
        }
        if (grabbing&&!IsDrag)
        {
            IsArea = false;
        ...
        else if(IsArea && IsDrag&&!grabbing)
        {
    ...
    void OnCollisionEnter(Collision collisionInfo)
    {
        ...
        //if(!that.TryGetComponent<CustomTag>(out customTag))
        //{
        //    touching = null;
        //    return;
        //}
        //if (customTag.tag != "GrabberItem")
        //    return;
        //if (IsCollision(transform, that.transform) == false)
        //    return;
               touching = that;
    }
    bool IsCollision(Transform self,Transform target)
    {
     Transform sphere=   self.Find("Sphere");
         Vector3[] vetrtices = sphere.GetComponent<MeshFilter>().mesh.vertices;
        Vector3 centerPoint = transform.position;
           Vector3[] collisionPoints = new Vector3[target.transform.childCount];
        bool isCollision = false;
        for (int i = 0; i < target.transform.childCount; i++)
        {
            //if((centerPoint-target.GetChild(i).position).magnitude<=redius)
            //{
            //    isCollision = true;
            //    break;
            //}
            collisionPoints[i] = target.transform.GetChild(i).position;
        }
        int count = vetrtices.Length;
     Matrix4x4 lw2 = sphere.localToWorldMatrix;
        bool b = false;
        Parallel.For(0, count, (i) =>
        {

            vetrtices[i]= lw2.MultiplyPoint(vetrtices[i]);
            

        });
        //Parallel.For(0, count, (i, ParallelLoopState) =>
        //{
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            {
                if ((vetrtices[i] - collisionPoints[j]).magnitude <= redius && b == false)
                {
                    b = true;
