...
namespace NVIDIA.Flex
{
    ...
    public class _auxFlexDetectShapes : MonoBehaviour
    {
        ...
        void OnTriggerEnter(Collider collider)
        {
            ...
            //modified by js
            if (collider.name.Contains("碎块"))
                return;
            if (collider.name.Contains("Cocuy_Fluid_Right"))
                return;
            if (collider.name.Contains("l_1"))
                return;
            if (collider.name.Contains("point"))
                return;
            if (collider.name.Contains("BezierSpline"))
                return;
            if (collider.name.Contains("Start"))
                return;
            if (collider.name.Contains("End"))
                return;
            if (collider.name.Contains("Highlight"))
                return;
