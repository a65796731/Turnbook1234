...
public class Triangulation
{
    ...
    {
        //by zwj 将原来的for循环改成并行计算效率提高非常之大

        // int i;
        this.lowerAngle = 1f;
        int vlen = meshC.sharedMesh.vertices.Length;
        this.vertices = new List<Vertex>(new Vertex[vlen]);
        Vector3[] vetxes = meshC.sharedMesh.vertices;
        Vector3[] normals = meshC.sharedMesh.normals;
        Vector2[] uvs = meshC.sharedMesh.uv;
        Parallel.For(0, vlen, (i) =>
        {
            Vector2 uvp = Vector2.zero;
            if(uvs.Length>i)
            {
                uvp = uvs[i];
            }
            this.vertices[i] = new Vertex(vetxes[i], 0, normals[i], uvp);
        }
        );
        int[] triangles = meshC.sharedMesh.triangles;
        int tlen = triangles.Length / 3;
        this.triangles = new List<Polygon>(new Polygon[tlen]);
        Parallel.For(0, tlen, (j) =>
        {
            this.triangles[j] = new Polygon(new int[3] { triangles[j * 3 + 0], triangles[j * 3 + 1], triangles[j * 3 + 2] });
        }
        );
    }
    ...
    public void updateLocalPosition(Transform matrix)
    {
        ...
        {
            this.vertices[i].localPos = matrix.worldToLocalMatrix.MultiplyPoint3x4(this.vertices[i].pos);
        }
    }
    public void updateWorldPosition(Transform matrix)
    {
        for (int i = 0; i < this.vertices.Count; i++)
        {
            this.vertices[i].pos = matrix.localToWorldMatrix.MultiplyPoint3x4(this.vertices[i].localPos);
        }
    }
