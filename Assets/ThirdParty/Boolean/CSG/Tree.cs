	...
	public class PolygonTreeNode
	{
 		...
 		void SplitPolygonByPlane(Plane plane, ref PolygonTreeNodeList coplanarfrontnodes, ref PolygonTreeNodeList coplanarbacknodes, ref PolygonTreeNodeList frontnodes, ref PolygonTreeNodeList backnodes)
		{
			...
			if (polygon != null)
			{
				...
				if (d > sphereradius)
				{
					if (frontnodes == null) frontnodes = CSGObjectPool.Instance.Pop();
					frontnodes.Add(this);
				}
				else if (d < -sphereradius)
				{
					if (backnodes == null) backnodes = CSGObjectPool.Instance.Pop();
					backnodes.Add(this);
				}
				else {
					...
					{
						case 0:
							if (coplanarfrontnodes == null) coplanarfrontnodes = CSGObjectPool.Instance.Pop();
							coplanarfrontnodes.Add(this);
							break;
						case 1:
							if (coplanarbacknodes == null) coplanarbacknodes = CSGObjectPool.Instance.Pop();
		...
		{
			var newchild = CSGObjectPool.Instance.PopNode();
   ...
   public	class PolygonTreeNodeList
	{
		...
		public PolygonTreeNode this[int index] =>
			nodes != null? nodes[index]	: (node0 );//throw new ArgumentOutOfRangeException (nameof (index))
