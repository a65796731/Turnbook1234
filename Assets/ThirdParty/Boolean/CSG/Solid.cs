...
namespace Csg
{
	public class Solid
	{
		...
		public (Solid, Solid) Substract(params Solid[] csgs)
		{
			Solid result = this;
			(Solid, Solid) result1=default;
			for (var i = 0; i < csgs.Length; i++)
			{
				var islast = (i == (csgs.Length - 1));
				result1 =  result.SubtractSub(csgs[i], islast, islast);
			}
			return result1;
		}
		...
		//	async Task< Solid> SubtractSub(Solid csg, bool retesselate, bool canonicalize)
		//	{
		//		if (false)
		//			await Task.Delay(0);
		//		staticText.inst.text.text = "";
		//		  //DateTime start = DateTime.Now;
		//		  a = new Tree(Bounds, Polygons);
		//           b = new Tree(csg.Bounds, csg.Polygons);
		//	//	DateTime end = DateTime.Now;
		//		//Debug.Log("Tree：" + (end - start).TotalMilliseconds);
		//	//	staticText.inst.text.text += "Tree：" + (end - start).TotalMilliseconds+"\n";
		//   //   start = DateTime.Now;
		//		a.Invert();
		//	//	 end = DateTime.Now;
		//		//Debug.Log("Invert：" + (end - start).TotalMilliseconds);
		//	//	staticText.inst.text.text += "Invert：" + (end - start).TotalMilliseconds + "\n";
		//	//	start = DateTime.Now;
		//		a.ClipTo(b,false);
		//	//	end = DateTime.Now;
		//		//Debug.Log("AClipTo：" + (end - start).TotalMilliseconds);
		//	//	staticText.inst.text.text += "AClipTo：" + (end - start).TotalMilliseconds + "\n";
		////		start = DateTime.Now;
		//		b.ClipTo(a, true);
		//	//	end = DateTime.Now;
		//		//Debug.Log("BClipTo：" + (end - start).TotalMilliseconds);
		//	//	staticText.inst.text.text += "BClipTo：" + (end - start).TotalMilliseconds + "\n";
		//	//	start = DateTime.Now;
		//		a.AddPolygons(b.AllPolygons());
		////		end = DateTime.Now;
		//	//	Debug.Log("AddPolygons：" + (end - start).TotalMilliseconds);
		//	//	staticText.inst.text.text += "AddPolygons：" + (end - start).TotalMilliseconds + "\n";
		////		start = DateTime.Now;
		//		a.Invert();
		...
		//           //	end = DateTime.Now;
		//           //	Debug.Log("FromPolygons：" + (end - start).TotalMilliseconds);
		//       //    staticText.inst.text.text += "FromPolygons：" + (end - start).TotalMilliseconds + "\n";
		//          DateTime    start = DateTime.Now;
		//         //  if (retesselate) result = result.Retesselated();
		//		DateTime end = DateTime.Now;
		//           Debug.Log("Retesselated：" + (end - start).TotalMilliseconds);
		//           staticText.inst.text.text += "Retesselated：" + (end - start).TotalMilliseconds + "\n";
		//           start = DateTime.Now;
		//        //		   if (canonicalize) result = result.Canonicalized();
		//           end = DateTime.Now;
		//           Debug.Log("Canonicalized：" + (end - start).TotalMilliseconds);
		//           staticText.inst.text.text += "Canonicalized：" + (end - start).TotalMilliseconds + "\n";
		//           return result;
		//	}
		 (Solid,Solid) SubtractSub(Solid csg, bool retesselate, bool canonicalize)
		{
	...
	//a.AddPolygons(b.AllPolygons());
			...
			//		start = DateTime.Now;
			var result = Solid.FromPolygons(a.AllPolygons());
			var result1 = Solid.FromPolygons(b.AllPolygons());
            //	end = DateTime.Now;
            //	Debug.Log("FromPolygons：" + (end - start).TotalMilliseconds);
            //    staticText.inst.text.text += "FromPolygons：" + (end - start).TotalMilliseconds + "\n";
            //DateTime start = DateTime.Now;
           result = result.Retesselated();
            ...
            result1 = result1.Retesselated();
            result1 = result1.Canonicalized();
            for (int i=0;i< result1.Polygons.Count;i++)
            {
				result1.Polygons[i].Flags = 2;
		...
		static void RetesselateCoplanarPolygons(List<Polygon> sourcepolygons, List<Polygon> destpolygons)
		{
			...
			if (numpolygons > 0)
			{
				...
				var polygon2flag= new Dictionary<int, int>();
				...
				for (var polygonindex = 0; polygonindex < numpolygons; polygonindex++)
				{
					...
					if (numvertices > 0)
					{
						...
						for (var i = 0; i < numvertices; i++)
						{
							...
							vertices2d.Add(new Vertex2D (pos2d, poly3d.Vertices[i].Tex, uv));
							var y = pos2d.Y;
							if ((i == 0) || (y < miny))
							{
								miny = y;
						...
						if (miny >= maxy)
						{
							...
							if(!polygon2flag.ContainsKey(polygonindex))
                            {
								polygon2flag.Add(polygonindex, flag);
				...
				for (var yindex = 0; yindex < ycoordinates.Count; yindex++)
				{
					...
					if (true)
					{
						...
						for (var activepolygon_key = 0; activepolygon_key < activepolygons.Count; activepolygon_key++)
						{
							...
							var topleft = new Vertex2D(x.Result, ycoordinate, x.Tex, x.Uv);
							x = InterpolateBetween2DPointsForY(activepolygon.topright, activepolygon.bottomright, ycoordinate);
							var topright = new Vertex2D(x.Result, ycoordinate, x.Tex, x.Uv);
							x = InterpolateBetween2DPointsForY(activepolygon.topleft, activepolygon.bottomleft, nextycoordinate);
							var bottomleft = new Vertex2D(x.Result, nextycoordinate, x.Tex, x.Uv);
							x = InterpolateBetween2DPointsForY(activepolygon.topright, activepolygon.bottomright, nextycoordinate);
							var bottomright = new Vertex2D(x.Result, nextycoordinate, x.Tex, x.Uv);
							var outpolygon = new RetesselateActivePolygon
							{
								flag = polygon2flag[polygonindex],
								topleft = topleft,
								topright = topright,
							...
							for (var ii = 0; ii < prevoutpolygonrow.Count; ii++)
							{
								if (!prevcontinuedindexes.Contains(ii))
								{
									...
									foreach (var point2d in points2d)
									{
										...
										var vertex3d = new Vertex(point3d, point2d.UV, point2d.Tex);
										vertices3d.Add(vertex3d);
									}
								
									var polygon = new Polygon(prevpolygon.flag,vertices3d, shared, plane);
		...
		static Vertex2DInterpolation InterpolateBetween2DPointsForY (Vertex2D vertex1, Vertex2D vertex2, double y)
		{
			...
			var uv = new Vector2D(0, 0);
			uv.X = vertex1.UV.X + t * (vertex1.UV.X - vertex1.UV.X);
			uv.Y = vertex1.UV.Y + t * (vertex1.UV.Y - vertex1.UV.Y);
		...
		struct Vertex2D
		{
			...
			public Vector2D Tex;
			public Vector2D UV;
			public Vertex2D (Vector2D pos, Vector2D tex,Vector2D uv)
			{
				Pos = pos;
				Tex = tex;
				UV = uv;
			}
			public Vertex2D (double x, double y, Vector2D tex, Vector2D uv)
			{
	...
	class FuzzyCsgFactory
	{
		...
		public Polygon GetPolygon(Polygon sourcepolygon)
		{
			...
			return new Polygon(flag,newvertices_dedup, newshared, newplane);
		}
